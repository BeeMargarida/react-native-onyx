...
function keysChanged(collectionKey, partialCollection) {
    ...
    // const stateMappingKeys = _.pairs(_.mapObject(callbackToStateMapping, obj => obj.key));
    // const sortedStateMappingKeys = _.map(_.sortBy(stateMappingKeys, ([k, v]) => {
    //     const index = _.indexOf(onyxKeysOrder, v);
    //     return index < 0 ? Infinity : index;
    // }), ([k, v]) => k);

    // console.log("[LOG] [Onyx](keysChanged) stateMappingKeys", stateMappingKeys);
...
function keyChanged(key, data, canUpdateSubscriber) {
    ...
    // console.log("[LOG] [Onyx](keyChanged) callbackToStateMapping", callbackToStateMapping);
...
function set(key, value) {
    if (freezeUpdates) {
        console.log('[LOG] [Onyx](set) freezed update', key, value);
        return;
    }
    ...
    console.log('[LOG] [Onyx](set) isLeader?', ActiveClientManager.isClientTheLeader());
...
function multiSet(data) {
    if (freezeUpdates) {
        console.log('[LOG] [Onyx](multiSet) freezed update', JSON.stringify(data));
        return;
    }

    const keyValuePairs = prepareKeyValuePairsForStorage(data);

    console.log('[LOG] [Onyx](multiSet) isLeader?', ActiveClientManager.isClientTheLeader());
    if (!ActiveClientManager.isClientTheLeader()) {
...
function merge(key, changes) {
    if (freezeUpdates) {
        console.log('[LOG] [Onyx](merge) freezed update', key, changes);
        return;
    }
...
function clear(keysToPreserve = []) {
    ...
    return getAllKeys()
        .then((keys) => {
            ...
            // @TODO: remove
            // Sorts keys taking into account the order defined when initializing Onyx
            // const sortedIndividualKeys = _.sortBy(_.keys(keyValuesToResetIndividually), (key) => {
            //     const index = _.indexOf(onyxKeysOrder, key);
            //     return index < 0 ? Infinity : index;
            // });

            // Notify the subscribers for each key/value group so they can receive the new values
            _.each(keyValuesToResetIndividually, (key) => {
                const value = keyValuesToResetIndividually[key];
            ...
            freezeUpdates = false;
            onClearCallback();
            broadcast.sendMessage({type: 'onClear'});
        });
}
